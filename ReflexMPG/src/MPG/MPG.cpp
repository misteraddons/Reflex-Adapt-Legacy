...
SwitchReport *MPG::getSwitchReport(const uint8_t index)
{
	...
	switchReport.right_axis = (state[index].dpad & GAMEPAD_MASK_RIGHT) * 0xFF;
	switchReport.left_axis =  (state[index].dpad & GAMEPAD_MASK_LEFT)  * 0xFF;
	switchReport.up_axis = 	  (state[index].dpad & GAMEPAD_MASK_UP)    * 0xFF;
	switchReport.down_axis =  (state[index].dpad & GAMEPAD_MASK_DOWN)  * 0xFF;

	switchReport.triangle_axis = pressedB4(index) * 0xFF;
	switchReport.circle_axis = 	 pressedB2(index) * 0xFF;
	switchReport.cross_axis = 	 pressedB1(index) * 0xFF;
	switchReport.square_axis = 	 pressedB3(index) * 0xFF;
	switchReport.l1_axis = pressedL1(index) * 0xFF;
	switchReport.r1_axis = pressedR1(index) * 0xFF;
	//switchReport.l2_axis = pressedL2(index) * 0xFF;
	//switchReport.r2_axis = pressedR2(index) * 0xFF;

	if (hasAnalogTriggers[index])
	{
		switchReport.l2_axis = state[index].lt;
		switchReport.r2_axis = state[index].rt;
	}
	else
	{
		switchReport.l2_axis = pressedL2(index) * 0xFF;
...
HIDReport *MPG::getHIDReport(const uint8_t index)
{
	...
	hidReport.buttons = 0
		...
		//Extra buttons
		| (pressedButtonAux(index, 1 << 0) ? (BTN_C)     : 0)
		| (pressedButtonAux(index, 1 << 1) ? (BTN_Z)     : 0)
		| (pressedButtonAux(index, 1 << 2) ? (1UL << 15) : 0)
		| (pressedButtonAux(index, 1 << 3) ? (1UL << 16) : 0)
		| (pressedButtonAux(index, 1 << 4) ? (1UL << 17) : 0)
		| (pressedButtonAux(index, 1 << 5) ? (1UL << 18) : 0)
		| (pressedButtonAux(index, 1 << 6) ? (1UL << 19) : 0)
		| (pressedButtonAux(index, 1 << 7) ? (1UL << 20) : 0)
		| (pressedButtonAux(index, 1 << 8) ? (1UL << 21) : 0)
		| (pressedButtonAux(index, 1 << 9) ? (1UL << 22) : 0)
	;
	...
	// 	else
	// 		hidReport.trigger = HID_JOYSTICK_MID;
	// 	// hidReport.lt = pressedL2(index) * 0xFF;
	// 	// hidReport.rt = pressedR2(index) * 0xFF;
	// }
...
XInputReport *MPG::getXInputReport(const uint8_t index)
{
	...
	{
		xinputReport.lt = pressedL2(index) * 0xFF;
		xinputReport.rt = pressedR2(index) * 0xFF;
	}
...
GunconReport *MPG::getGunconReport(const uint8_t index)
{
	gunconReport.buttons = 0
		| (pressedB1(index) ? 0x1 : 0)
		| (pressedB2(index) ? 0x2 : 0)
		| (pressedB3(index) ? 0x4 : 0)
		//| (pressedB4(index) ? BTN_Y : 0)
		// | (pressedL1(index) ? BTN_TL      : 0)
		// | (pressedR1(index) ? BTN_TR      : 0)
		// | (pressedL2(index) ? BTN_TL2     : 0)
